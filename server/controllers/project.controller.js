...
module.exports.create = (req, res) => {
    ...
    if (payload) {
        ...
        Project.create(project)
            .then(data => {
                Project.findById(data._id).populate('user', '-password')
...
module.exports.get = (req, res) => {
    Project.findById(req.params.id).populate('user', '-password')
...
module.exports.list = (req, res) => {
    Project.find().populate('user', '-password')
        .then(data => res.status(200).json({ ok: true, message: 'project', data: data }))
        .catch(error => {
            console.log('LIST', error);
            res.status(500).json({ ok: false, message: 'Error al obtener los proyectos' })
        });
}

module.exports.del = (req, res) => {
    Project.findByIdAndRemove(req.params.id)
        .then(data => res.status(200).json({ ok: true, message: 'Se eliminó  el projecto', data: data }))
        .catch(error => {
